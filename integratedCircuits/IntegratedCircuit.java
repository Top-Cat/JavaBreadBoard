...
/*   33 */   protected String name = getClass().getSimpleName();
/*      */   protected String description;
/*      */   protected String manufacturer;
/*      */   protected String diagram;
/*   49 */   protected boolean wide = false;
/*      */ 
/*   54 */   protected List<Pin> pins = new ArrayList();
/*      */ 
...
/*  640 */       if (((Pin)this.pins.get(i)).getPinNumber() == pinNumber)
/*  641 */         return i;
/*      */     }
...
/*  654 */       if (((Pin)this.pins.get(i)).getPinName().equals(pinName))
/*  655 */         return i;
/*      */     }
...
/*  672 */       return ((Pin)this.pins.get(pinIndex)).getPinName();
...
/*  684 */       return ((Pin)this.pins.get(pinIndex)).getPinNumber();
...
/*  700 */       Pin pin = (Pin)this.pins.get(pinIndex);
...
/*  721 */       Pin pin = (Pin)this.pins.get(pinIndex);
...
/*  742 */       Pin pin = (Pin)this.pins.get(pinIndex);
...
/*  763 */       Pin pin = (Pin)this.pins.get(pinIndex);
...
/*  784 */       Pin pin = (Pin)this.pins.get(pinIndex);
...
/*  805 */       Pin pin = (Pin)this.pins.get(pinIndex);
/*      */ 
...
/*  839 */       Pin pin = (Pin)this.pins.get(pinIndex);
/*      */ 
...
/*  873 */       Pin pin = (Pin)this.pins.get(pinIndex);
/*      */ 
...
/*  907 */       Pin pin = (Pin)this.pins.get(pinIndex);
/*      */ 
...
/*      */     throws InvalidPinException
/*      */   {
/*  949 */     return getPinNumber(pinName) - 1;
/*      */   }
/*      */ 
/*      */   public int[] getPinOffsets(String pinName)
/*      */     throws InvalidPinException
/*      */   {
/*  959 */     Pin[] pins = getPins(pinName);
...
/*      */     throws InvalidPinException
/*      */   {
/* 1156 */     if (this.chip.readTTL(getPinOffset(input)).equals("HIGH")) {
/* 1157 */       getPin(input).setPinState(Pin.PinState.HIGH);
/*      */ 
/* 1159 */       return true;
/*      */     }
/*      */ 
/* 1162 */     getPin(input).setPinState(Pin.PinState.LOW);
/*      */ 
/* 1164 */     return false;
/*      */   }
/*      */ 
...
/*      */     throws InvalidPinException
/*      */   {
/* 1179 */     if (this.chip.readTTL(getPinOffset(input)).equals("HIGH")) {
/* 1180 */       getPin(input).setPinState(Pin.PinState.HIGH);
/*      */ 
/* 1182 */       return false;
/*      */     }
/*      */ 
/* 1185 */     getPin(input).setPinState(Pin.PinState.LOW);
/*      */ 
/* 1187 */     return true;
/*      */   }
/*      */ 
...
/*      */     throws InvalidPinException
/*      */   {
/* 1202 */     if (getPin(input).getPinState().equals(Pin.PinState.HIGH))
/*      */     {
/* 1204 */       return true;
/*      */     }
/*      */ 
...
/*      */     throws InvalidPinException
/*      */   {
/* 1223 */     if (getPin(input).getPinState().equals(Pin.PinState.LOW))
/*      */     {
/* 1225 */       return true;
/*      */     }
/*      */ 
...
/*      */     throws InvalidPinException
/*      */   {
/* 1244 */     return getPin(input).getPinState();
/*      */   }
/*      */ 
/*      */   public boolean isRisingEdge(String input)
/*      */     throws InvalidPinException
/*      */   {
/* 1258 */     Pin.PinState currentState = getPinState(input);
/* 1259 */     if ((isHigh(input)) && (currentState.equals(Pin.PinState.LOW)))
/*      */     {
/* 1261 */       return true;
/*      */     }
/*      */ 
...
/*      */     throws InvalidPinException
/*      */   {
/* 1280 */     Pin.PinState currentState = getPinState(input);
/* 1281 */     if ((isLow(input)) && (currentState.equals(Pin.PinState.HIGH)))
/*      */     {
/* 1283 */       return true;
/*      */     }
/*      */ 
...
/*      */     {
/*      */       String pin;
/* 1331 */       if (isPinOffsetInput(pinOffset)) {
/* 1332 */         pin = "IN";
/*      */       }
/*      */       else
/*      */       {
/* 1334 */         if (isPinOffsetClockOutput(pinOffset)) {
/* 1335 */           pin = "CLO";
/*      */         }
/*      */         else
/*      */         {
/* 1337 */           if (isPinOffsetInputOutput(pinOffset)) {
/* 1338 */             pin = "IO";
/*      */           }
/*      */           else
/*      */           {
/* 1340 */             if ((isPinOffsetOpenCollector(pinOffset)) || (isPinOffsetTriState(pinOffset))) {
/* 1341 */               pin = "OCO";
/*      */             }
/*      */             else
/*      */             {
/* 1343 */               if (isPinOffsetOutput(pinOffset)) {
/* 1344 */                 pin = "OUT";
/*      */               }
/*      */               else
/*      */               {
/* 1346 */                 if (isPinOffsetPower(pinOffset))
/* 1347 */                   pin = "IN"; 
/*      */               }
/*      */             }
/*      */           }
...
/*      */ 
/* 1371 */     String className = getClass().toString();
/* 1372 */     className = className.substring(6);
/*      */ 
/* 1374 */     String pathName = getClass().toString().replace('.', '/');
...
/*      */ 
/* 1386 */       for (int i = 0; i < files.length; i++)
/*      */       {
/* 1389 */         if (files[i].endsWith(".class")) {
/*      */           try {
/* 1391 */             Object thisObject = Class.forName(className);
/*      */ 
/* 1393 */             String newName = (pathName + "/" + files[i].substring(0, files[i].length() - 6)).replace('/', '.');
/*      */             try {
...
/*      */     throws InvalidPinException
/*      */   {
/* 1427 */     getPin(output).setPinState(state);
/* 1428 */     switch (state.ordinal()) {
/*      */     case 1:
/* 1430 */       this.chip.write(getPinOffset(output), "HIGH", ((OutputPin)getPin(output)).getPinDelayTplh());
/*      */ 
/* 1432 */       break;
/*      */     case 2:
/* 1434 */       this.chip.write(getPinOffset(output), "LOW", ((OutputPin)getPin(output)).getPinDelayTphl());
/*      */ 
/* 1436 */       break;
/*      */     case 3:
/* 1438 */       this.chip.write(getPinOffset(output), "NC", 0);
/*      */ 
/* 1440 */       break;
/*      */     case 4:
/* 1442 */       this.chip.write(getPinOffset(output), "NC", 0);
/*      */ 
/* 1444 */       break;
/*      */     case 5:
/* 1446 */       break;
...
/*      */     throws InvalidPinException
/*      */   {
/* 1465 */     getPin(output).setPinState(state);
/* 1466 */     int riseFallDelay = ((OutputPin)getPin(output)).getPinDelayTplh();
/* 1467 */     int timeDelay = riseFallDelay + delay;
/*      */ 
/* 1469 */     switch (state.ordinal()) {
/*      */     case 1:
/* 1471 */       this.chip.write(getPinOffset(output), "HIGH", timeDelay);
/*      */ 
/* 1473 */       break;
/*      */     case 2:
/* 1475 */       this.chip.write(getPinOffset(output), "LOW", timeDelay);
/*      */ 
/* 1477 */       break;
/*      */     case 3:
/* 1479 */       this.chip.write(getPinOffset(output), "NC", timeDelay);
/*      */ 
/* 1481 */       break;
/*      */     case 4:
/* 1483 */       this.chip.write(getPinOffset(output), "NC", timeDelay);
/*      */ 
/* 1485 */       break;
/*      */     case 5:
/* 1487 */       break;
 ...
 * Qualified Name:     integratedCircuits.IntegratedCircuit
